    ...
    for row_index, row in enumerate(data):
        for column_index, character in enumerate(row):
            if character.isdigit():
                temporal_number.append(character)
                temporal_column.append(column_index)
                if column_index + 1 < len(row):
                    continue
            else:
                if len(temporal_number) == 0:
                    continue
            number = int(''.join(temporal_number))
            size = len(temporal_number)
            digits.append((number, size, row_index, min(temporal_column), max(temporal_column)))
            temporal_number = []
            temporal_column = []

    # [(digit, size, row, column_start, column_end)]
    return digits


def find_symbols(data):
    symbols = []
    not_symbols = get_not_symbols()
    for row_index, row in enumerate(data):
        for column_index, character in enumerate(row):
            if ord(character) not in not_symbols:
                symbols.append((character, row_index, column_index))
...
def get_not_symbols():
    ...
    # Special characters
    not_symbols += list(range(0, 33))
    # Delete
    not_symbols.append(127)
    # Period
    not_symbols.append(46)

    # Digits
    not_symbols += list(range(48, 58))

    # Alphabet uppercase
    not_symbols += list(range(65, 91))

    # Alphabet lowercase
    not_symbols += list(range(97, 123))

    return not_symbols


def match_in_row(row, column_start, column_end, symbols):
    for symbol in symbols:
        symbol_row = symbol[1]
        symbol_column = symbol[2]
        if row == symbol_row:
            if symbol_column in range(column_start - 1, column_end + 2):
                return True

    return False


def match_previous_row(digit, symbols):
    # Check the row index not lower than zero.
    if digit[2] == 0:
        return False

    # Previous row
    row = digit[2] - 1
    column_start = digit[3]
    column_end = digit[4]

    return match_in_row(row, column_start, column_end, symbols)


def match_current_row(digit, symbols):
    row = digit[2]
    column_start = digit[3]
    column_end = digit[4]
...
def match_next_row(digit, max_rows, symbols):
    # Check the row index not lower than zero.
    if digit[2] == max_rows:
        return False
...
def match_digits_and_symbols(digits, symbols):
    match_digits = []

    for symbol in symbols:
        if symbol[0] == '*':
            print(f'symbol: {symbol}')
            symbol_range = [symbol[2] - 1, symbol[2], symbol[2] + 1]
            print(f'symbol_range: {symbol_range}')
            match_digits_symbol = []
            for digit in digits:
                if digit[2] < symbol[1] - 1 or digit[2] > symbol[1] + 1:
                    continue
                print(f'digit: {digit}')
                digit_range = list(range(digit[3], digit[4] + 1))
                # print(f'digit_range: {digit_range}')
                for symbol_position in symbol_range:
                    # print(f'symbol_position: {symbol_position}')
                    if symbol_position in digit_range:
                        match_digits_symbol.append(digit[0])
                        print(f'match_digits_symbol: {match_digits_symbol}')
                        break

            if len(match_digits_symbol) > 1:
                multiplication = 1
                for match_digit in match_digits_symbol:
                    print(f'match_digit: {match_digit}')
                    multiplication *= match_digit
                print(f'multiplication: {multiplication}')
                match_digits.append(multiplication)
            print()

    return match_digits


digits = find_digits(engine_schematic)
# [(digit, size, row, column_start, column_end)]
# print(f'digits: {digits}')

symbols = find_symbols(engine_schematic)
# [(symbol, row, column)]
# print(f'symbols: {symbols}')

match_digits = match_digits_and_symbols(digits, symbols)
print(f'match_digits({len(match_digits)}): {match_digits}')

sum_matches = sum(match_digits)
print(f'sum_matches: {sum_matches}')
