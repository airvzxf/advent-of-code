...
def match_digits_and_symbols(digits, symbols):
    ...
    match_digits = []
    not_match_digits = []
    max_rows = len(digits) - 1
    for digit in digits:
        all_digits.append(digit[0])
        if match_previous_row(digit, symbols):
            match_digits.append(digit[0])
            continue
        elif match_current_row(digit, symbols):
            match_digits.append(digit[0])
            continue
        elif match_next_row(digit, max_rows, symbols):
            match_digits.append(digit[0])
            continue
        not_match_digits.append(digit[0])

    return match_digits, not_match_digits, all_digits
...
matches, not_matches, all_digits = match_digits_and_symbols(digits, symbols)
print(f'matches ({len(matches)}): {matches}')
print(f'not_matches({len(not_matches)}): {not_matches}')
print(f'all_digits({len(all_digits)}): {all_digits}')
